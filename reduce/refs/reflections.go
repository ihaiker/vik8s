...
// FieldNames returns the struct fieldNames names list. obj can whether
// be a structure or pointer to structure.
func FieldNames(obj interface{}) ([]string, error) {
	return fieldNames(obj, false)
}

// FieldNamesDeep returns "flattened" fieldNames (fieldNames from anonymous
// inner structs are treated as normal fieldNames)
func FieldNamesDeep(obj interface{}) ([]string, error) {
	return fieldNames(obj, true)
}

func fieldNames(obj interface{}, deep bool) ([]string, error) {
	if !hasValidType(obj, []reflect.Kind{reflect.Struct, reflect.Ptr}) {
		return nil, errors.New("Cannot use GetField on a non-struct interface")
	}
	...
	for i := 0; i < fieldsCount; i++ {
		...
		if isExportableField(field) {
			if deep && field.Anonymous {
				...
				subFields, err := fieldNames(fieldValue.Interface(), deep)
				if err != nil {
					return nil, fmt.Errorf("Cannot get fieldNames in %s: %s", field.Name, err.Error())
				}
...
// FieldNamesDeep returns "flattened" items (fieldNames from anonymous
// inner structs are treated as normal fieldNames)
...
// Tags lists the struct tag fieldNames. obj can whether
// be a structure or pointer to structure.
func Tags(obj interface{}, key string) (map[string]string, error) {
	return tags(obj, key, false)
}

// FieldNamesDeep returns "flattened" tags (fieldNames from anonymous
// inner structs are treated as normal fieldNames)
...
func isExportableField(field reflect.StructField) bool {
	// PkgPath is empty for exported fieldNames.
